...
import { HelpModal } from './HelpModal'; // Importe seu componente de ajuda

// Reaproveitando a estrutura de setores
const SECTORS = {
...
export function DeviceEditModal({ isOpen, onClose, deviceId }: DeviceEditModalProps) {
  ...
  const fetchDevice = useCallback(async () => {
    ...
    setLoading(true);
    try {
        const token = localStorage.getItem('@App:token'); // Pega o token
        const response = await axios.get(`http://192.168.1.3:8087/api/devices/${deviceId}`, {
        headers: { Authorization: `Bearer ${token}` } // Envia no header
        });
  ...
  // 2. Atualizar Dispositivo (PUT)
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    try {
      await axios.put(`http://192.168.1.3:8087/api/devices/${deviceId}`, formData);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
      ...
      <div onClick={(e) => e.stopPropagation()} className="bg-white mt-10 2xl:mt-36 dark:bg-zinc-900 w-full max-w-6xl rounded-3xl shadow-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in duration-200 my-auto">
        ...
        <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-900/50 shrink-0">
            <div className="flex items-center gap-3">
                ...
                {/* <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest leading-none">ID: {deviceId?.split('-')[0]}</p> */}
                </div>
            </div>
          <div className="flex items-center gap-6">
                ...
                <button
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, isBeingUsed: !prev.isBeingUsed }))}
          ...
          <div className="col-span-full flex flex-col md:flex-row justify-between items-center gap-4 mt-1 2xl:mt-8 pt-1 2xl:pt-6 2xl:border-t 2xl:border-zinc-100 2xl:dark:border-zinc-800">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto">
              ...
              <button 
                type="submit"
                disabled={loading}
                className="flex-1 md:flex-none cursor-pointer flex items-center justify-center gap-2 bg-primary dark:bg-secondary text-white dark:text-zinc-900 px-10 py-3 rounded-xl font-black uppercase tracking-tighter hover:scale-105 transition-all shadow-lg disabled:opacity-50"
              >
